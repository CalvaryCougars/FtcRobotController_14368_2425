...
//---------------------------------------------------------------------------------
    private static final double COUNTS_PER_MOTOR_REV = 537.6; // GoBilda 5202 Series Motor (encoder ticks per revolution)
    private static final double WHEEL_DIAMETER_INCHES = 3.779;  // Mecanum wheel diameter in inches
...
//---------------------------------------------------------------------------------
    ...
    private GoBildaPinpointDriver pinpoint; //fix no usages must use in order to get "odo"
    ...
    public void rotate(int degree, double pwr) {
         ...
         //fr fl br bl
         public void diagonal (double distance, double v,double vB) { 
            // because t_final = t_initial + distance / v
            final double t_running = clock.seconds() + Math.abs(distance / (v * Max_motor_speed)); 
         ...
         public void rotMove (double distance, double fr, double fl, double br, double bl) {
             
        final double t_running = clock.seconds() + Math.abs(distance  / (fr  * Max_motor_speed));
        while (clock.seconds() < t_running) { 
                drive( fr, fl, br, bl);
            } 
         ...
         public void sideRotate(double dis,double p)
         {
             final double t_running = clock.seconds() + Math.abs(dis  / (p  * Max_motor_speed));
             //t_running = clock.seconds() + Math.abs(disR  / (p  * Max_motor_speed));
             //(double fr, double fl, double br, double bl)
             while (clock.seconds() < t_running) {
             drive(-p,p,-p,p);
             }
        ...
        wheelBr.setPower(0);
         ...
         // Method to strafe left or right with encoders
        public void strafe(double distanceInInches, double power) {
        int targetPosition = (int) (distanceInInches * COUNTS_PER_INCH);
        // Set target positions for strafing
        wheelFl.setTargetPosition(wheelFl.getCurrentPosition() + targetPosition);
        wheelBl.setTargetPosition(wheelBl.getCurrentPosition() - targetPosition);
        wheelFr.setTargetPosition(wheelFr.getCurrentPosition() - targetPosition);
        wheelBr.setTargetPosition(wheelBr.getCurrentPosition() + targetPosition);
        ...
        setRunMode(DcMotor.RunMode.RUN_USING_ENCODER); // Switch back to normal mode
        }
        // Method to move forward/backward with encoders
